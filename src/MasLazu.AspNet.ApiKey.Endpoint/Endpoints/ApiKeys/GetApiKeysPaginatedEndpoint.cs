...
public class GetApiKeysPaginatedEndpoint : BaseEndpoint<PaginationRequest, PaginatedResult<ApiKeyDto>>
{
    ...
    public override async Task HandleAsync(PaginationRequest request, CancellationToken ct)
    {
        ...
        PaginatedResult<ApiKeyDto> result = await ApiKeyService.GetPaginatedAsync(userId, request, ct);
        await SendOkResponseAsync(result, "Api keys retrieved successfully", ct);
    }
}
