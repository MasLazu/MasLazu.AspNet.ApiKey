...
public class RotateApiKeyEndpoint : BaseEndpoint<IdRequest, ApiKeyDto>
{
    ...
    public override async Task HandleAsync(IdRequest request, CancellationToken ct)
    {
        ...
        ApiKeyDto result = await ApiKeyService.RotateAsync(userId, request.Id, ct);
        await SendOkResponseAsync(result, "Api key rotated successfully", ct);
    }
}
