...
public class UseApiKeyEndpoint : BaseEndpointWithoutResponse<IdRequest>
{
    ...
    public override async Task HandleAsync(IdRequest request, CancellationToken ct)
    {
        ...
        await ApiKeyService.UpdateLastUsedAsync(userId, request.Id, ct);
        await SendOkResponseAsync("Api key usage updated", ct);
    }
}
